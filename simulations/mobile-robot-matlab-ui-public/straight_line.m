...
if time < my_alg('t_finish')    % Check for algorithm finish time
    ...
    if dt>my_alg('t_sampling')  % execute code when desired sampling time is reached
        ...
        theta = (linearVelocity_left - linearVelocity_right) / 0.18; %angle is clockwise
        my_alg('total_theta') = theta + my_alg('total_theta');
...
%            end
        ...
        my_alg('wR_set') = forwardspeed;%= linearvelocity-0.09*phi;
        my_alg('wL_set') = forwardspeed;%= linearvelocity+0.09*phi;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        my_alg('wR_all') = [my_alg('wR_all') forwardspeed];
        my_alg('wL_all') = [my_alg('wL_all') averageVelocity];
